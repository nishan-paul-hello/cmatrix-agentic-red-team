...
\addplot[fill=blue!60] coordinates {(Web, 82) (Network, 88) (Auth, 74) (Overall, 81)};
...
\legend{CMatrix, PentestGPT, HackingBuddy, SOTA (2025) \cite{checkmate2025}}
\end{axis}
\end{tikzpicture}
\caption{Task Completion Success Rates on NYU CTF Bench ($N=200$ challenges). CMatrix significantly outperforms 2024 baselines and remains competitive with the 2025 SOTA (CHECKMATE).}
\label{fig:results}
\end{figure}
...
Our evaluation demonstrates that CMatrix achieves an 81\% task completion rate on the NYU CTF Bench while maintaining an absolute safety record through its checkpoint-suspended approval mechanism. This performance demonstrates that "Governance-First Autonomy" is achievable; security agents can be delegated complex, multi-stage objectives without sacrificing operational control. CMatrix provides a foundational architecture for the safe transition from human-led penetration testing to autonomous, AI-orchestrated threat simulation in enterprise environments.
