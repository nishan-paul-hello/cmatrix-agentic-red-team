...
The quest to automate the penetration testing lifecycle has been reinvigorated by the reasoning capabilities of Large Language Models. PentestGPT \cite{pentestgpt} was among the first to demonstrate that LLMs could guide a human operator through complex HackTheBox environments, though it remains a "guided" system rather than a fully autonomous executor. HackingBuddyGPT \cite{hackingbuddygpt, hackingbuddy} extended this by allowing an agent to execute shell commands for Linux privilege escalation, showing that GPT-4 can outperform traditional automated scripts. More recently, AutoAttacker \cite{autoattacker} and the work of Fang et al. \cite{fang2024} have demonstrated that agents can autonomously exploit real-world one-day vulnerabilities with success rates exceeding 80\%.

However, as summarized in Table~\ref{tab:comparison}, these systems are primarily offensive prototypes. They lack the formal safety governance required for production environments, possess no longitudinal memory across engagement sessions, and rely on monolithic architectures that struggle with specialized security toolsets. CMatrix is designed to address these three deficiencies systematically.
...
HackingBuddy \cite{hackingbuddy} & \texttimes & \texttimes & \texttimes & \texttimes & \texttimes & \texttimes & \texttimes & \texttimes \\
AutoAttacker \cite{autoattacker} & \texttimes & \texttimes & \texttimes & \texttimes & Partial & \texttimes & \texttimes & \texttimes \\
AutoGen \cite{autogen} & Partial & \texttimes & \checkmark & \texttimes & \texttimes & \texttimes & \texttimes & \texttimes \\
