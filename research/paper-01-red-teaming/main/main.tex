...
\begin{abstract}
The emergence of Large Language Model (LLM) agents has enabled autonomous multi-step penetration testing; however, existing academic prototypes lack formal safety governance, presenting a critical barrier to production deployment. This paper introduces \textbf{CMatrix}, a production-safe multi-agent framework for autonomous red teaming. We present three primary technical contributions: (1) a \textbf{hierarchical Human-in-the-Loop (HITL) safety framework} utilizing a formal four-level risk taxonomy and deterministic auto-rejection guardrails, integrated directly into the agent control graph via checkpoint interrupts; (2) a \textbf{keyword-confidence Supervisor Router} that enables dynamic, multi-strategy task delegation across specialized agent subgraphs; and (3) an \textbf{agentic RAG memory pipeline} that utilizes cross-encoder reranking to maintain longitudinal threat intelligence. Our evaluation on the NYU CTF Bench demonstrates that CMatrix achieves competitive task completion ($81\% \pm 3.4\%$) while enforcing absolute safety recall across all destructive adversarial payloads. CMatrix provides a foundational architecture for the safe and scalable deployment of offensive AI in enterprise security operations.
\end{abstract}
...
The remainder of this paper is organized as follows. Section~\ref{sec:related} reviews the related work in autonomous penetration testing and LLM safety. Section~\ref{sec:threat} formalizes our threat model and safety properties. Section~\ref{sec:arch} details the CMatrix system architecture. Section~\ref{sec:safety} describes our hierarchical HITL safety framework. Section~\ref{sec:reasoning} details our reasoning and self-reflection loops. Section~\ref{sec:memory} presents our agentic RAG memory pipeline. Section~\ref{sec:eval} presents our experimental evaluation and results. Section~\ref{sec:discussion} discusses the implications and limitations, and Section~\ref{sec:conclusion} concludes the paper.
...
CMatrix utilizes a formal four-level risk taxonomy---\textit{Low}, \textit{Medium}, \textit{High}, and \textit{Critical}---to categorize every tool in the agent's registry. Tools are assigned a \textit{RiskLevel} based on their potential impact on the target system's availability, integrity, and confidentiality.
...
\legend{CMatrix, PentestGPT, HackingBuddy, SOTA (2025)}
\end{axis}
\end{tikzpicture}
\caption{Task Completion Success Rates on NYU CTF Bench. Error bars indicate standard deviation over $N=10$ runs ($p < 0.05$). CMatrix maintains competitive performance with the 2025 SOTA while providing 100\% safety recall.}
\label{fig:results}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Cross-Session Memory Performance ($N=10$ runs). Agentic RAG enables CMatrix to accumulate intelligence across engagement sessions, achieving a 42\% success rate delta by Session 5.}
\label{fig:rag_gain}
\end{figure}
...
We identify three primary directions for the evolution of CMatrix. First, the integration of \textit{Explainable Safety} (X-Safety) would provide natural-language justifications for high-risk tool calls, mitigating operator alert fatigue. Second, we plan to transition from deterministic keyword routing to a \textit{learned routing model} to improve adaptability in ambiguous environments. Finally, we aim to incorporate \textit{Formal Verification} of the safety control graph to mathematically prove that no execution path can bypass the mandatory oversight gate under specified adversarial conditions.
