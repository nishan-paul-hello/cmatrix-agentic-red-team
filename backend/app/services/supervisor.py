...
class SupervisorService:
    ...
    def analyze_task(self, task: str) -> Dict[str, Any]:
        ...
        network_keywords = [
            "scan", "port", "network", "nmap", "host", "ip", "ping",
            "tcp", "udp", "service", "open ports", "network security"
        ]
        ...
        # Web agent keywords
        web_keywords = [
            "web", "http", "https", "ssl", "tls", "header", "cookie",
            "website", "url", "domain", "certificate", "cors", "csp"
        ]
        ...
        vuln_keywords = [
            "cve", "vulnerability", "exploit", "nvd", "threat",
            "security advisory", "patch", "weakness", "exposure"
        ]
    ...
    def aggregate_results(
        ...
        logger.info(f"Aggregation complete: {status} ({len(successful_results)}/{len(results)} agents succeeded)")
        return aggregated
            ...
            if isinstance(agent_result, dict) and "messages" in agent_result:
                ...
                if messages:
                    ...
                    if i == 0:
                        ...
                        # Supporting results
                        answer_parts.append(f"### Additional Insights from {agent_name}\n\n{content}")
    ...
    async def supervise(
        ...
        if strategy == DelegationStrategy.SINGLE:
            ...
            results = [await self.delegate_to_agent(
                agents_to_use[0],
                task,
                context,
                llm_provider,
