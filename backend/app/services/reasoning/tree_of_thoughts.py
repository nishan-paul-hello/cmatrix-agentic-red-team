...
class Strategy:
    ...
    def calculate_overall_score(
        self,
        weights: Optional[Dict[EvaluationCriterion, float]] = None
    ) -> float:
        """
...
class StrategyEvaluation:
    ...
    def get_strategy_ranking(self) -> List[Tuple[Strategy, float]]:
        ...
        return sorted(
            [(s, s.overall_score) for s in self.strategies],
            key=lambda x: x[1],
            reverse=True
        )
...
class TreeOfThoughts:
    ...
    STRATEGY_TEMPLATES = {
        ...
        StrategyType.BALANCED: {
            ...
            "cons": [
                "Not optimal for any specific requirement",
    ...
    def evaluate_strategies(
        ...
        logger.info(
            f"✅ Selected: {selected.name} (score: {selected.overall_score:.2f})"
        )
    ...
    def _evaluate_strategy(
        ...
        for criterion in criteria:
            ...
            strategy.scores.append(StrategyScore(
                criterion=criterion,
                score=score,
                reasoning=reasoning
            ))
...
Provide concise explanation in 2-3 sentences."""
        ...
        comparison = "\n".join(
            f"{i+1}. {s.name}: {s.overall_score:.2f}"
