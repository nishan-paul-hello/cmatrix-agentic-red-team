...
class ABTestingFramework:
    ...
    def record_feedback(
        ...
        if feedback_type == FeedbackType.CLICK:
            ...
            variant.avg_position_clicked = (
                (variant.avg_position_clicked * (total_clicks - 1) + position) / total_clicks
            )
    ...
    def analyze_experiment(
        ...
        if control.impressions < experiment.min_sample_size:
            logger.warning(f"Insufficient samples for control: {control.impressions}/{experiment.min_sample_size}")
        if treatment.impressions < experiment.min_sample_size:
            logger.warning(f"Insufficient samples for treatment: {treatment.impressions}/{experiment.min_sample_size}")
        
        # Calculate metrics
        control_ctr = control.clicks / control.impressions if control.impressions > 0 else 0.0
        treatment_ctr = treatment.clicks / treatment.impressions if treatment.impressions > 0 else 0.0
        
        control_satisfaction = (
            (control.thumbs_up - control.thumbs_down) / 
            (control.thumbs_up + control.thumbs_down)
            if (control.thumbs_up + control.thumbs_down) > 0 else 0.0
        )
        treatment_satisfaction = (
            (treatment.thumbs_up - treatment.thumbs_down) / 
            (treatment.thumbs_up + treatment.thumbs_down)
            if (treatment.thumbs_up + treatment.thumbs_down) > 0 else 0.0
        )
        ...
        if SCIPY_AVAILABLE:
            ...
            control_positions = [
                e.position for e in self._feedback[experiment_id]
        ...
        if treatment_ctr > control_ctr * 1.05:  # 5% improvement threshold
            if not statistical_tests or statistical_tests.get("ctr_chi_square", {}).get("significant", False):
                winner = "treatment"
        ...
        results = {
            ...
            "comparison": {
                "ctr_lift": ((treatment_ctr - control_ctr) / control_ctr * 100) if control_ctr > 0 else 0.0,
                "position_improvement": control.avg_position_clicked - treatment.avg_position_clicked,
    ...
    def _generate_recommendation(
        ...
        if winner == "treatment":
            ...
            if statistical_tests and statistical_tests.get("ctr_chi_square", {}).get("significant", False):
