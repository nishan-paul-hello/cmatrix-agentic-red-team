...
class BackpressureConfig(BaseModel):
    ...
    batch_timeout_ms: int = Field(
        default=100,
        gt=0,
        description="Maximum time to wait for batch (milliseconds)"
    )
    max_events_per_second: int = Field(
        default=100,
        gt=0,
        description="Maximum events per second (rate limit)"
    )
    compression_threshold_bytes: int = Field(
        default=1024,
        gt=0,
        description="Compress payloads larger than this (bytes)"
    )
    max_buffer_size: int = Field(
        default=1000,
        gt=0,
        description="Maximum buffered events before blocking"
    )
    enable_compression: bool = Field(
        default=True,
        description="Enable gzip compression for large payloads"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "compression_savings_pct": (
                ...
                if self.total_bytes_sent > 0 else 0
            ),
        }
    ...
    async def batch(
        ...
        async for event in events:
            ...
            should_yield = (
                ...
                (time.time() - last_yield) >= self.timeout_s
            )
...
class BackpressureManager:
    ...
    def __init__(self, config: BackpressureConfig):
        ...
        self.rate_limiter = RateLimiter(
            max_rate=config.max_events_per_second
        )
