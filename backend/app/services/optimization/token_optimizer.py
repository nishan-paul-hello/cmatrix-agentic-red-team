...
class TokenOptimizerConfig(BaseModel):
    ...
    summarization_threshold: int = Field(
        default=20,
        gt=0,
        description="Summarize conversation after this many messages"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "savings_percentage": round(
                ...
                if (self.total_cost_usd + self.cost_saved_usd) > 0 else 0,
                2
            ),
        }
...
class PromptCompressor:
    ...
    FILLER_WORDS = {
        "please", "kindly", "basically", "actually", "literally",
    ...
    def _extractive_summarize(self, messages: List[BaseMessage]) -> str:
        ...
        if user_queries:
            summary_parts.append(
                f"User asked about: {'; '.join(user_queries[:3])}"
            )
        
        if ai_responses:
            summary_parts.append(
                f"Assistant discussed: {'; '.join(ai_responses[:3])}"
            )
...
class TokenOptimizer:
    ...
    def optimize_messages(
        ...
        # 3. Filter tools
        if (
            self.config.dynamic_tool_filtering and
            tools and
            query
        ):
            optimized_tools = self.filterer.filter_tools(query, tools)
            logger.info(
                f"Tool filtering: {len(tools)} → {len(optimized_tools)} tools"
            )
    ...
    def track_usage(
        ...
        # Update averages
        self.stats.avg_input_tokens = (
            self.stats.total_input_tokens / self.stats.total_requests
        )
        self.stats.avg_output_tokens = (
            self.stats.total_output_tokens / self.stats.total_requests
        )
        self.stats.avg_cost_per_request = (
