        ...
        try:
            ...
            if response.status_code >= 400:
                if 'stack' in response.text.lower() or 'trace' in response.text.lower():
                    results.append(f"  ⚠️  Potential stack trace disclosure")
            
        except requests.Timeout:
            results.append(f"  ⚠️  Request timed out")
...
def check_api_authentication(api_url: str) -> str:
    ...
    try:
        ...
        results.append(f"Request without authentication:")
        results.append(f"  Status: {response_no_auth.status_code}")
        
        if response_no_auth.status_code == 200:
            results.append(f"  ⚠️  API accessible without authentication!")
        elif response_no_auth.status_code == 401:
            results.append(f"  ✅ Authentication required (401)")
        elif response_no_auth.status_code == 403:
            results.append(f"  ✅ Access forbidden (403)")
        ...
        if 'WWW-Authenticate' in response_no_auth.headers:
            ...
            if 'Basic' in auth_method:
                results.append(f"  ⚠️  Basic authentication detected - ensure HTTPS is used")
            elif 'Bearer' in auth_method:
                results.append(f"  ✅ Bearer token authentication")
...
def test_graphql_introspection(graphql_url: str) -> str:
    ...
    try:
        ...
        results.append(f"Introspection Query Test:")
        results.append(f"  Status: {response.status_code}")
        ...
        if response.status_code == 200:
            try:
                ...
                if 'data' in data and '__schema' in data['data']:
                    ...
                    results.append(f"  ⚠️  Introspection is ENABLED")
                    results.append(f"  Found {len(types)} schema types")
                    results.append(f"  ⚠️  This exposes your entire API schema")
                else:
                    results.append(f"  ✅ Introspection appears to be disabled")
            except:
                results.append(f"  ℹ️  Could not parse response")
        else:
            results.append(f"  ✅ Introspection query rejected")
...
API_SECURITY_TOOLS = [test_api_endpoints, check_api_authentication, test_graphql_introspection, check_api_rate_limiting]
