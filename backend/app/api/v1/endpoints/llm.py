...
async def get_profiles(
    ...
    try:
        ...
        for profile in profiles:
            ...
            result.append(ConfigurationProfileSchema(
                id=profile.id,
...
async def create_profile(
    ...
    try:
        profile = await config_profile_service.create_profile(
            db, current_user.id, request.name, request.api_provider,
            request.api_key, request.selected_model_name
        )
...
async def update_profile(
    ...
    try:
        profile = await config_profile_service.update_profile(
            ...
            request.name, request.api_provider, request.api_key, request.selected_model_name
        )
...
async def activate_profile(
    ...
    try:
        profile = await config_profile_service.activate_profile(
            db, profile_id, current_user.id
        )
...
async def fetch_models(
    ...
    try:
        models = await api_provider_service.fetch_models(
            request.api_provider, request.api_key
        )
