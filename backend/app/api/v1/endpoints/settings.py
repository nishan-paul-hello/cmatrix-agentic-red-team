...
async def get_nvd_api_key(
    ...
    result = await db.execute(
        select(SystemSetting).where(SystemSetting.key == "nvd_api_key")
    )
    ...
    if not setting:
        return SettingResponse(
            key="nvd_api_key",
            value=None,
            description="NVD API key for CVE synchronization"
        )
...
async def delete_nvd_api_key(
    ...
    try:
        await db.execute(
            delete(SystemSetting).where(SystemSetting.key == "nvd_api_key")
        )
        await db.commit()
