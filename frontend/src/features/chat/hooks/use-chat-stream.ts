  ...
  const sendMessage = useCallback(
    async (userMessage: string) => {
      ...
      try {
        ...
        const createResponse = await fetch(`${apiConfig.baseUrl}${apiConfig.endpoints.jobs.create}`, {
          method: "POST",
          headers: {
            ...apiConfig.headers,
        ...
        while (pollCount < MAX_POLLS) {
          ...
          const statusResponse = await fetch(`${apiConfig.baseUrl}${apiConfig.endpoints.jobs.get(job_id)}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
          ...
          if (statusResponse.status === 410) {
            ...
            setMessages(prev => {
              ...
              updated[updated.length - 1] = {
                role: "assistant",
                content: "Your request took too long and the result has expired. Please try sending your message again.",
              };
              return updated;
            });
        ...
        setMessages((prev) => {
          ...
          if (lastMsg && lastMsg.role === "assistant") {
            updated[updated.length - 1] = {
              ...lastMsg,
              content: lastMsg.content && !lastMsg.content.startsWith("Processing")
                ? `${lastMsg.content}\n\n[Error: ${errorMessage}]`
                : errorMessage,
            };
            return updated;
