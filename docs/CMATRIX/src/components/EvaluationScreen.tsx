  ...
  return (
    <div className="flex flex-col h-full" style={{ minHeight: 0 }}>
      <div className="flex-shrink-0 px-6 pt-5 pb-4" style={{ borderBottom: "1px solid #1E1E1E" }}>
        <div style={{ fontSize: 9, color: "#666666", letterSpacing: "0.22em", marginBottom: 3 }}>EXECUTION / {entry.nodeId}</div>
        <div className="flex items-baseline gap-3">
          <h1 style={{ fontSize: 20, fontWeight: 700, color: "#F2F2F2", letterSpacing: "0.12em" }}>EVALUATION</h1>
          <span style={{ fontSize: 9, color: "#D29922", background: "#1A1200", border: "1px solid #D2992244", borderRadius: 2, padding: "1px 7px", letterSpacing: "0.14em" }}>EXECUTION #{entry.execId}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6" style={{ maxWidth: 760 }}>
        ...
        {[
          { label: "WHAT HAPPENED",      color: "#A0A0A0", content: entry.whatHappened      },
          { label: "EXPECTED VS ACTUAL", color: "#A0A0A0", content: entry.expectedVsActual  },
          { label: "NEXT STEP",          color: "#A0A0A0", content: entry.nextStep           },
        ].map((block, i, arr) => (
          <div key={block.label}>
            ...
            <div className="flex justify-between">
              {EORD_LABELS.map((lbl, i) => (
                <div key={lbl} className="flex flex-col items-center" style={{ zIndex: 1 }}>
                  ...
                  <div style={{ position: "absolute", bottom: 0, fontSize: 7.5, color: i === VALUE ? "#E31B23" : "#333333", letterSpacing: "0.1em", whiteSpace: "nowrap",
