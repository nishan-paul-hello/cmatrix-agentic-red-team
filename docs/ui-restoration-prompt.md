...
- **Mode Cards**: Extend the Surface Cards pattern (removing internal dividers, `text-[10px]` sub-headers, `text-primary` selected title, tight tag padding) to complex multi-row radio cards for consistent density. — first applied on missions/new (Step 4)
- **Mode Cards**: For key-value lists inside cards (like implications), use `text-[10px] uppercase tracking-widest` for keys and `text-xs leading-relaxed` for values to match the density of sub-headers. — first applied on missions/new (Step 4)
- **Review Panels**: Apply the same `text-xs font-semibold uppercase tracking-widest` styling from block labels to summary table keys and panel headers (like PRE-FLIGHT CHECKS) for consistent hierarchy density. — first applied on missions/new (Step 5)
- **Data Tables**: Action buttons in table headers (like + NEW MISSION) should use `text-[10px] font-semibold tracking-widest` rather than `text-base` or `text-sm` to maintain the tight density of the header row. — first applied on dashboard
- **Data-Dense Surfaces**: Log/activity feeds (like EXECUTION LOG) should use `text-[10px]` with `font-semibold` and `tracking-widest uppercase` for labels/columns to match tight density. — first applied on missions/NEW-001
