...
**Revision note (this version):** Four gaps identified during internal review are addressed below, each admitted only where it produces a testable, citable contribution rather than a cosmetic addition: (1) a concrete Tool Catalogue and Tool Adapter Layer (§3.3) — the previous draft named tools inconsistently in prose; (2) a Live Vulnerability-Intelligence Grounding function (§5.8) closing the stale-pretrained-knowledge gap that Fang et al.'s own CVE-description ablation identifies; (3) an explicit graded Tool Risk Gate and lifecycle hook system (§5.9) giving mechanism to the safety-framing claim in §5.6; (4) formalization of the existing Handoff Bridge/ESS pattern as a named context-compaction contribution (§5.10). One idea considered and deliberately **not** adopted — splitting the VDG into two separate graphs (discovered-fact vs. inferred-opportunity) — is addressed in §5.1 instead, via a node-status field, to avoid contradicting the single-unified-structure novelty claim in §5.4.
...
flowchart TD
    ...
    subgraph L3["Layer 3 — Specialists (fresh context per task)"]
        Recon["Recon Specialist\n(nmap -p- -sV, Amass, httpx,\nWhatWeb, ObserverWard, ZAP-as-mapper)"]
        SQLi["SQLi Specialist\n(sqlmap-driven baseline→SLEEP probe→\nbit extraction FSM)"]
        XSS["XSS Specialist\n(ZAP active scan seed +\n5-phase: canary→context→\nmutation→verify→webhook)"]
        ...
        Lateral["Lateral-Movement Specialist\n(Incalmo-style declarative tasks over\nImpacket/CrackMapExec:\nScan, LateralMove, EscalatePrivilege,\nFindInfo, Exfiltrate)"]
        Intel["Intelligence Specialist\n(scoped NVD / Exploit-DB / GitHub\nlookup — §5.8)"]
    end

    subgraph L3b["Tool Adapter Layer (§3.3)"]
        RiskGate["Tool Risk Gate\nLow → execute\nMedium → LLM permission classifier\nHigh → Team Manager mailbox"]
        Adapters["Tool Adapters\n(parse raw stdout/HTTP →\nstructured finding JSON;\nnothing unparsed reaches L2/VDG)"]
    end
    ...
    ExtIntel[("External Intelligence Sources\nNVD · Exploit-DB · GitHub advisories")]

    Operator --> Intake --> Recon0 --> VDG
    VDG --> ADM --> Dispatch --> L3
    L3 --> RiskGate --> Adapters --> Exec --> Eval --> Val
    Intel -->|"scoped outbound\nrequests only"| ExtIntel
    Intel -->|"enriched CVE/PoC\nattributes"| Adapters
    Val -->|"finding JSON\n{type,severity,confidence,evidence,status}"| Handoff
    Handoff --> VDG
    ...
    Hooks["Lifecycle Hooks (§5.9)\nPreToolUse · PostToolUse · PreSpawn\nPostReturn · PreVDGUpdate · PostMissionEnd"]
    Hooks -.->|"named interception points,\nno agent-logic changes"| L2
    Hooks -.-> L3b
```
...
- **Structured Handoff Bridge**: every Specialist's raw stdout/HTTP response is compressed into a structured summary before re-entering the Team Manager's context (D-CIPHER + VulnBot Summarizer pattern) — this is what prevents the context-flooding pathology identified as the *architectural*, not capability, bottleneck of single-agent systems. This mechanism is formalized as an explicit compaction contribution in §5.10.

### 3.3 Tool Catalogue and Tool Adapter Layer

Every Specialist calls tools **only** through the Tool Adapter Layer (see architecture diagram) — never directly. This closes the gap in the previous draft, where tools were named inconsistently in prose and had no single point of normalization or risk classification.

| Tool | Category | Specialist(s) | Role |
|---|---|---|---|
| **nmap** (`-p- -sV`) | Recon | Recon | Full-depth port/service scan — default to full-surface, not top-1000, per §5.5(c) |
| **Amass** | Recon | Recon | Subdomain enumeration (DNS brute-force, CT logs, passive OSINT) |
| **httpx** | Recon | Recon | Live-host probing across discovered subdomains |
| **WhatWeb** / **ObserverWard** | Recon | Recon | Technology/CMS/framework fingerprinting |
| **OWASP ZAP** (mapper + active-scan mode) | Analysis | Recon, XSS, AuthSess | Passive crawl as Recon's surface mapper; active scan as XSS/CSRF/auth seed |
| **Gobuster** / **ffuf** | Analysis | Recon, GraphQL | Directory/file/parameter brute-forcing; API route and vhost fuzzing |
| **Nuclei** | Analysis | Recon | Template-based CVE/misconfiguration matching against fingerprinted services |
| **sqlmap** | Validation | SQLi | Baseline injection detection, SLEEP-probe timing, bit-extraction FSM driver |
| **Custom XSS harness** | Validation | XSS | 5-phase canary→context→mutation→verify→webhook pipeline (no off-the-shelf equivalent in the survey covers this pattern) |
| **PrediQL-derived GraphQL fuzzer** | Validation | GraphQL | Introspection-driven schema abuse, dependency-chain mutation/query fuzzing |
| **Metasploit** | Validation | SQLi, AuthSess | Known-exploit execution once a CVE/weakness is confirmed |
| **Impacket / CrackMapExec** | Validation | Lateral | Credential reuse, lateral movement, privilege escalation (Incalmo declarative verbs execute through these) |
| **EyeWitness** | Evidence | Validation Agent (L4) | Headless screenshot capture for PoC/impact evidence |
| **NVD API / Exploit-DB / GitHub advisories** | Intelligence | Intelligence Specialist | Live CVE/PoC/version grounding (§5.8) — read-only, never executes against the target |

**Tool Adapter contract**: each adapter (a) executes the tool with Team Manager-issued parameters, (b) parses raw output into a structured finding (`{type, severity, confidence, evidence, status}` — see §5.1's updated VDG schema), (c) never re-interprets or summarizes with an LLM call at this layer — parsing is deterministic/regex-and-schema-based, consistent with the "Execution Agent never reasons" rule in §8. Nothing unparsed ever reaches Layer 2 or the VDG.

**Tool Risk Gate** (detailed in §5.9) classifies every call — Low (passive discovery, e.g. nmap/Amass/httpx), Medium (active enumeration, e.g. Gobuster/ffuf/Nuclei/ZAP-active), or High (destructive/exploitative, e.g. sqlmap write-mode, Metasploit, Impacket lateral-move) — before the Execution Agent runs it.

### 3.4 Layer 3 — Specialists
...
### 3.5 Layer 4 — Execution & Validation
...
### 3.6 Memory & State Services (cross-cutting)
...
EGATS-style systems search well but don't encode formal prerequisites; PentestEval-style systems encode prerequisites but were evaluated with pre-enumerated, expert-curated weakness sets — they don't scale to open-ended discovery. CMatrix's VDG node schema fuses both: `{weakness_id, vuln_class, prerequisites[], enables[], priority (UCB score), attack_intent, promise φ, TDI δ, evidence E, status}` — a scored, dynamically-growing DAG that is populated by open-ended Specialist discovery (solving PentestEval's scalability gap) and pruned/ranked by UCB search (solving EGATS's lack of formal dependency semantics).

**On `status` and why this is a field, not a second graph.** An alternative design considered during review was splitting the VDG into two separate graphs — one holding only confirmed discovered facts, one holding only inferred attack reasoning — mirroring how several non-VAPT agent-memory systems separate "world state" from "plan state." This was deliberately **not** adopted: CMatrix's central novelty claim (§5.4, §10-4) is that *one unified planning structure* generalizes across web, GraphQL, and multi-host surfaces, and introducing a second graph would blur that claim without a corresponding ablation to justify the added complexity. Instead, the same discipline — never conflating an observed fact with an attack hypothesis — is enforced with a typed `status ∈ {DISCOVERED, HYPOTHESIZED, PARTIALLY_VALIDATED, VALIDATED, RULED_OUT}` field on every VDG node, with a write-ownership rule identical in spirit to a graph split: Specialists may only write nodes at `DISCOVERED`; only the Team Manager's ADM step may advance a node to `HYPOTHESIZED`; only the Validation Agent (L4) may advance a node to `PARTIALLY_VALIDATED`/`VALIDATED`/`RULED_OUT`. This gets the fact/hypothesis separation property without paying for a second data structure or diluting the single-structure novelty claim.
...
### 5.8 Live Vulnerability-Intelligence Grounding as a Fix for the Stale-Knowledge Gap

Fang et al.'s own ablation is direct evidence for this addition: removing the CVE description from the agent's context collapses one-day performance, because the LLM's pre-trained knowledge of a specific CVE is frequently stale, incomplete, or absent for recent disclosures. No system in the 29-paper survey closes this gap with a live retrieval step scoped *during* a mission rather than only at prompt-construction time.

The **Intelligence Specialist** is a narrowly-scoped Layer-3 agent, spawned by the Team Manager whenever a Recon or Analysis finding produces a fingerprinted technology/version string or an unresolved CVE reference. It queries three authorized sources only — NVD (CVSS vector, affected version ranges), Exploit-DB (public PoC existence and type), GitHub advisories/PoC repos — and returns a single enriched attribute set (`severity`, `poc_available`, `exploit_type`, `recommended_validation_approach`) written back onto the relevant VDG node via the Handoff Bridge. It is the **only** Specialist authorized to make outbound requests to non-target infrastructure; this boundary is enforced at the Tool Risk Gate (§5.9), not by convention. Raw retrieved content never enters any agent's context — only the structured, adapter-parsed intelligence record does, consistent with the Handoff Bridge discipline in §3.2.

This directly strengthens one-day-mode performance (where a CVE hint exists but may be under-specified) and partially compensates zero-day mode (where fingerprinted-technology lookups can surface historically-exploited weakness classes for that stack even without a CVE hint). **Ablation**: with/without Intelligence Specialist, held constant across Tier 0/0b, isolates this contribution the same way the four ablations in §7 isolate the others.

### 5.9 Graded Tool Risk Gate and Lifecycle Hooks

§5.6 claims economic and safety-framing metrics as co-primary but, in the previous draft, gave no mechanism for controlling *when* a risky call executes — only how its refusal rate is reported after the fact. This is fixed with two additions, both cheap to implement and both strengthening §5.6's contribution rather than competing with it:

**Tool Risk Gate.** Every tool call reaching the Adapter Layer (§3.3) is classified before execution:
- **Low** (passive discovery — nmap, Amass, httpx, WhatWeb): executes immediately after a scope check (target is within declared rules of engagement).
- **Medium** (active enumeration — Gobuster, ffuf, Nuclei, ZAP active-scan): evaluated by a fast, narrowly-scoped **LLM permission classifier** — a single call to the same configured LLM API (§8), issued with a constrained classification-only prompt — checking scope alignment, VDG-node/attack-intent consistency, and parameter-safety (e.g. flagging aggressive timing on a mode that didn't authorize it). Output is binary: `EXECUTE` or `ESCALATE`.
- **High** (destructive/exploitative — sqlmap write operations, Metasploit, Impacket lateral-move): always routes to the Team Manager's approval mailbox with the proposed tool, parameters, target VDG node, and rationale attached. For fully autonomous missions the Team Manager approves per protocol rules already encoded in its scoring function; for supervised missions a human operator can be inserted at the same mailbox with zero change to agent logic — this is the same mechanism as the existing Human Escalation path in the architecture diagram, generalized to pre-execution rather than only post-failure.

This gives the refusal-rate metric in §5.6 an actual point of measurement (Medium-tier escalation rate, High-tier approval rate) instead of only a citation to BountyBench's observation.

**Lifecycle hooks.** Six named interception points — `PreToolUse`, `PostToolUse`, `PreSpawn`, `PostReturn`, `PreVDGUpdate`, `PostMissionEnd` — give the same operator-intervention and observability capability the diagram already implies (Human Escalation, Cost tracking) a formal, addressable structure. Each hook receives a structured event payload and returns `CONTINUE`, `BLOCK`, or `MODIFY(payload)`. This is not a new capability so much as naming and exposing capabilities the architecture already requires (scope checks, escalation, cost tracking) as stable integration points — relevant for the reproducibility package (§7) and for framing CMatrix as usable in supervised/enterprise deployments, which strengthens the systems-paper angle for USENIX/CCS reviewers without adding evaluation surface.

### 5.10 ASG/ESS-Backed Context Compaction as an Explicit Contribution

The Structured Handoff Bridge (§3.2) and Environment State Service already implement the core idea — raw tool/Specialist output is compressed before re-entering the Team Manager's context, and durable findings live in the VDG/ESS rather than in conversation history — but the previous draft treated this only as an implementation detail supporting other claims. It is formalized here as its own testable claim: **conversation history is expendable; VDG/ESS state is not.** Because every discovered fact and every hypothesis is persisted as VDG/ESS state (not only in the Team Manager's running context), the Team Manager's context can be aggressively compacted between planning cycles — summarized to near-zero — without losing any finding, since any compacted-away detail remains queryable from VDG/ESS on demand. This is measurable directly: token count in the Team Manager's context per planning cycle, with/without aggressive compaction, checked against whether compaction causes any measurable drop in downstream pass rate (it shouldn't, if the claim holds) — a fifth ablation alongside the four in §7.

### 5.11 Self-Consistency Verification at the ADM Decision Point

PentestEval's own ablation (§1, §5.1) identifies ADM as the stage with the largest marginal contribution to end-to-end success — which makes it the single highest-leverage point in the architecture to add a verification step, rather than spreading verification budget evenly across all stages. When the VDG's UCB scoring produces a close call between two or more candidate nodes (top-two UCB scores within a configurable margin, e.g. 10%), the Team Manager issues **k independent ADM scoring calls** (self-consistency, k=3 by default) instead of one, and selects the majority/highest-confidence outcome rather than a single sample. This is applied selectively — only at close calls, not on every ADM decision — to keep the added cost bounded and directly tied to the failure mode it targets. **Ablation**: single-sample vs. self-consistency ADM, isolated the same way as the other four ablations in §7, with cost-per-additional-pass@1-point reported so the tradeoff is explicit rather than assumed favorable.
...
| Agents reason only from stale pre-trained CVE/exploit knowledge, no live grounding during a mission | Most surveyed systems (only Fang et al. diagnoses this via CVE-description ablation, no system fixes it) | Intelligence Specialist with scoped NVD/Exploit-DB/GitHub retrieval, written back as VDG node attributes (§5.8) |
| No graded, pre-execution risk classification for tool calls; safety framing measured only after the fact | All surveyed systems report refusal rate post-hoc at best; none gate execution by risk tier | Tool Risk Gate (Low/Medium-classifier/High-mailbox) + named lifecycle hooks (§5.9) |
| Verification budget spread evenly across planning decisions rather than targeted at the highest-leverage stage | All surveyed planning-first and search-first systems | Self-consistency sampling applied selectively at close-call ADM decisions only (§5.11) |
| No reusable, labeled dataset of autonomous VAPT reasoning traces released alongside results | All 29 surveyed systems | Structured per-mission trajectory export (§10-8), released as a reproducibility + dataset artifact |
...
- Ablations required for the paper's core claims: (1) VDG vs. flat dispatch (isolates §5.1's contribution), (2) with/without 3-tier memory and skill promotion (isolates §5.3), (3) with/without Classical-Planning+ skeleton (isolates §5.2), (4) with/without alternative-surface queue + meta-critic (isolates §5.5), (5) with/without Intelligence Specialist live grounding (isolates §5.8), (6) with/without self-consistency ADM at close calls, reported with cost-per-additional-pass@1-point (isolates §5.11).
- Context-compaction check (§5.10): Team Manager context-token count per planning cycle with/without aggressive compaction, checked against downstream pass rate to confirm compaction is lossless as claimed, not merely cheaper.
...
| Tool Risk Gate — Medium tier classifier | Cheap/fast model, constrained classification-only prompt | Fast-filter-plus-brief-CoT pattern (§5.9); this is a narrow binary decision, not open-ended reasoning, so a frontier model is unnecessary overhead |
| Intelligence Specialist | Mid-tier model | Retrieval-and-structuring task, not novel reasoning; correctness depends more on source coverage than model capability |
| Self-consistency ADM samples (close calls only, §5.11) | Same tier as VDG scoring | k=3 independent samples of the *same* frontier-tier call, not a separate cheaper verifier — the value being tested is sampling variance reduction, not a different model's judgment |
...
- **Component-level evidence does not guarantee composition-level gains.** Every mechanism in §5 is individually motivated by a result in the survey, but no surveyed paper combines all of them — VDG scoring, hybrid planning, 3-tier memory, parallel exploration queues, live intelligence grounding, and self-consistency ADM interacting simultaneously could produce coordination overhead, conflicting signals, or context pressure that no single source paper's isolated result predicts. The four-plus-two ablations in §7 exist specifically to detect this; Tier 0/0b should be run before committing full Tier 2–6 compute, so any composition failure is caught cheaply.
- **Live intelligence grounding (§5.8) introduces an external dependency and a new failure mode**: NVD/Exploit-DB/GitHub availability, rate limits, or staleness at retrieval time are outside CMatrix's control and should be logged per-mission so a grounding failure is distinguishable from a reasoning failure in the trajectory export (§10-8).
...
8. **Structured per-mission trajectory export as a reproducibility and dataset artifact**: every planning-cycle step (VDG trigger, Team Manager scoring rationale, action taken, Specialist output, ablation-flag state) is logged in a machine-readable format, serving simultaneously as (a) full reproducibility support for every benchmark claim in §7, (b) the mechanism the six ablations in §7 are computed from, and (c) a publicly releasable, labeled dataset of autonomous VAPT reasoning sequences — a data-availability contribution increasingly rewarded independently of the architecture claims themselves.
9. **Live vulnerability-intelligence grounding**: a scoped Intelligence Specialist closes the stale-pretrained-knowledge gap Fang et al.'s own CVE-description ablation identifies, the only mechanism in the survey addressing this diagnostically-known but previously unfixed failure mode (§5.8).
10. **Graded, pre-execution safety architecture**: a Tool Risk Gate (Low/Medium-classifier/High-mailbox) and named lifecycle hooks give §5.6's economic/safety-framing claim an actual point of measurement rather than only a post-hoc refusal-rate citation (§5.9).

Target venue framing: a systems + empirical evaluation paper (USENIX Security / IEEE S&P / ACM CCS for the security-systems angle, or NDSS), with CVE-Bench, PrediQL, and Incalmo/MHBench as the three primary comparison points (one per benchmarked surface), the full Tier 0–6 suite plus the two additional ablations from §5.8/§5.11 as the reproducibility package, and the trajectory-export corpus (contribution 8) as a companion released dataset.
