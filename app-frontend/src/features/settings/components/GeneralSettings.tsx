    ...
    return (
        <div className="max-w-panel-xl flex-1 overflow-y-auto px-6 py-6">
            ...
            {[
                ...
                >
                    <span className="tracking-tight-1 text-lg text-[var(--color-hex-888888)]">
                        {s.l}
                    </span>
                    <Toggle on={s.on} />
                </div>
            ...
            <Field label="MAX RUNTIME">
                <div className="flex items-center gap-2 focus:border-[var(--color-brand)]">
                    <input
                        ...
                        className="font-inherit w-[72px] rounded-[2px] border-[1px] border-solid border-[var(--color-hex-1e1e1e)] bg-[var(--color-hex-0d0d0d)] px-[8px] py-[5px] text-right text-lg text-[var(--color-hex-a0a0a0)] outline-none"
                    />
                    <span className="text-base-tight text-[var(--color-hex-444444)]">hours</span>
                </div>
            </Field>
            <Field label="COST CEILING">
                <div className="flex items-center gap-2 focus:border-[var(--color-brand)]">
                    <input
                        ...
                        className="font-inherit w-[72px] rounded-[2px] border-[1px] border-solid border-[var(--color-hex-1e1e1e)] bg-[var(--color-hex-0d0d0d)] px-[8px] py-[5px] text-right text-lg text-[var(--color-hex-a0a0a0)] outline-none"
                    />
                    <span className="text-base-tight text-[var(--color-hex-444444)]">USD</span>
                </div>
            </Field>
            ...
            <Field label="ROE TEXT">
                <textarea
                    ...
                    className="font-inherit min-h-[80px] w-full rounded-[2px] border-[1px] border-solid border-[var(--color-hex-1e1e1e)] bg-[var(--color-hex-0d0d0d)] px-[12px] py-[10px] text-lg leading-loose tracking-tighter text-[var(--color-hex-888888)] outline-none"
                    style={{
                        resize: "vertical",
            ...
            <div className="rounded-[2px] border-[1px] border-solid border-[var(--color-hex-ff2a3244)] bg-[var(--color-hex-0a0605)] px-[16px] py-[14px]">
                <div className="tracking-wider-2 mb-[8px] text-base text-[var(--color-danger)]">
                    DESTRUCTIVE ACTIONS
                </div>
                <div className="flex gap-3">
                    {["CLEAR ALL MISSIONS", "RESET KNOWLEDGE BASE", "FACTORY RESET"].map((a) => (
                        <button
                            key={a}
                            className="font-inherit text-base-tight cursor-pointer rounded-[2px] border-[1px] border-solid border-[var(--color-hex-ff2a3266)] bg-[transparent] px-[10px] py-[5px] tracking-normal text-[var(--color-danger)]"
                        >
                            {a}
                        </button>
                    ))}
            ...
            <div className="mt-[24px]">
                <button
                    ...
                    className="font-inherit tracking-wider-1 cursor-pointer rounded-[2px] border-[1px] border-solid border-[var(--color-hex-1e1e1e)] bg-[transparent] px-[10px] py-[4px] text-sm text-[var(--color-hex-333333)]"
                >
                    {devRef ? "▾" : "▸"} DEV REFERENCE
                </button>
                {devRef && (
                    <div className="tracking-tight-1 mt-[10px] rounded-[2px] border-[1px] border-solid border-[var(--color-hex-1a1a1a)] bg-[var(--color-hex-0b0b0b)] px-[14px] py-[10px] text-base leading-loose text-[var(--color-hex-444444)]">
